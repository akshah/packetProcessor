...
typedef struct _patricia_node_data_t {
   uint16_t iff;
   uint16_t iff_cnt;
   uint16_t iff_list[50];
} patricia_node_data_t;
...
typedef struct _patricia_node_t {
   ...
   uint16_t iff; /* Interface for this node */
   uint16_t iff_cnt; /* Number of children. */
   uint16_t iff_list[50]; /* Interfaces of all descendants.*/
} patricia_node_t;
...
patricia_node_t *try_search_exact(patricia_tree_t *patricia, char *prefix);
